...
gCodeStart = m104(extruderTemperature) + m140(bedTemperature) + g17() + g21() + g28() + g90() + g92(x,y,z) + m82() + g0(0,0,10,travelSpeed) + m109(extruderTemperature) + m190(bedTemperature) + g1retractreversed(50) + g0(list[0],list[1],lineHeigth,travelSpeed)
for a in range (0,len(list) - 2,2):
    X = list[a]
    Y = list[a + 1]
    x = list[a + 2]
    y = list[a + 3]
    gCodeStart = gCodeStart + g1(x,y,z,extrusionSpeed,X,Y,lineWidth,lineHeigth,pi)
X = list[-2]
Y = list[-1]
x = list[0]
y = list[1]
gCodeStart = gCodeStart + g1(x,y,z,extrusionSpeed,X,Y,lineWidth,lineHeigth,pi) + g1retract(retractionDistance)
